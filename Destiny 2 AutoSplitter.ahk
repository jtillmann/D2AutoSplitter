...
loop, 4
{
    Hotkey%A_Index% := settingsArray[A_Index]
}
; ===================================================
...
Sethotkeys:
    ...
    {
        if (settingsArray[1] != "")
            Hotkey, % settingsArray[1], off
        settingsArray[1] := HotKey1
        Hotkey, $%HotKey1%, StartKeyPressed
    }
    if (Hotkey2 != "")
    {
        if (settingsArray[2] != "")
            Hotkey, % settingsArray[2], off
        settingsArray[2] := HotKey2
        Hotkey, $%HotKey2%, ResetAutoSplitter
    }
    if (Hotkey3 != "")
    {
        if (settingsArray[3] != "")
            Hotkey, % settingsArray[3], off
        settingsArray[3] := HotKey3
        Hotkey, $%HotKey3%, SkipSplit
    }
    if (Hotkey4 != "")
    {
        if (settingsArray[4] != "")
            Hotkey, % settingsArray[4], off
        settingsArray[4] := HotKey4
        Hotkey, $%HotKey4%, UndoSplit
    }
    if (CaptureHotkey != "")
    {
        if (settingsArray[5] != "")
            Hotkey, % settingsArray[5], off
        settingsArray[5] := CaptureHotkey
        Hotkey, $%CaptureHotkey%, Capture
    }
    hotkeySettingsString := settingsArray[1]"&"settingsArray[2]"&"settingsArray[3]"&"settingsArray[4]"&"settingsArray[5]
